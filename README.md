...
*   **麦克风校准**: 集成 pywebview 校准窗口，支持麦克风参数调优。
*   **侧边栏快览**: 实时展示今日朗读数据统计。
...
- **PyAudio 安装失败**: 在 Windows 上，可能需要手动下载 `.whl` 文件或安装 VC++ 工具
- **Web TTS 无法使用**: 请确保网络连接正常
- **pywebview 相关问题**: 某些 Linux 发行版可能需要额外的系统依赖
...
- 查看日志: 在数据面板中打开日志查看器
- 启用详细日志: 修改 `settings.json` 中的 `debug` 字段
...
**当前版本**: 3.0.0 | **最后更新**: 2026 年 4 月 19 日 | **状态**: 🟡 开发中